...
try:
    import ansible.module_utils.pg8000 as pg8000
except ImportError:
    HAS_PG8K = False
else:
    HAS_PG8K = True
...
def db_exists(cursor, db):
    query = "SELECT * FROM pg_database WHERE datname=(%s)"
    cursor.execute(query, [db])
...
#        "ssl_mode": "sslmode",
    ...
    if is_localhost and module.params["login_unix_socket"] != "":
        kw["unix_sock"] = module.params["login_unix_socket"]

    if target == "":
        target = "{0}/{1}.sql".format(os.getcwd(), db)
        target = os.path.expanduser(target)
    # module.fail_json(msg=f"{kw}")
    if not raw_connection:
        try:
            db_connection = pg8000.connect(database=maintenance_db, **kw)

            # Enable autocommit so we can create databases
            #if pg8000.__version__ >= '2.4.2':
            db_connection.autocommit = True
            #else:
            #    db_connection.set_isolation_level(pg8000.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
